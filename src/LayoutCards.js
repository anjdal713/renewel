...
// Import FontAwesome styles (adjust the version based on your preference)
    ...
    <div className="layout-cards">
      {cards.map((card, i) => (
        <motion.div
          ...
          key={i}
          layout
          whileTap={{ scale: 1.1 }} // Click effect
          onClick={() => handleCardClick(card)} // Handle click instead of pan
          ref={(el) => (containerRefs.current[card] = el)}
        >
          {selectedId === card && (
            <div>
              <h3>{items.find((item) => item.id === selectedId).title}</h3>
              <p>{items.find((item) => item.id === selectedId).subtitle}</p>
              <p>교수: {items.find((item) => item.id === selectedId).professor}</p>
              <button
      ...
      <AnimatePresence>
        {selectedId && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
          >
            {/* You can add content for the modal here if needed */}
          </motion.div>
        )}
      </AnimatePresence>
    </div>
