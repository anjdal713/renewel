...
const cards = [1, 2, 3, 4];
const items = [
  { id: 1, title: '모바일 소프트웨어 트랙', 
    engName:'Mobile Software',
  ...
  { id: 4, title: '디지털 콘텐츠 및 가상현실 트랙', 
    engName:'Digital Contents & Virtual Reality',
      ...
      <div className="layout-cards">
        {cards.map((card, i) => (
          ...
          >
            ...
            {selectedId !== card && (
              <div>
              <h3 className="h3-custom2">{items.find((item) => item.id === card).title}</h3>
              <p className="p-custom2">{items.find((item) => item.id === card).engName}</p>
            </div>
            )}
          </motion.div>
        ))}
